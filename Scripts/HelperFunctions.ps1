...
Param([string] $text, [string]$file)
    $secString = ConvertTo-SecureString -String $text -AsPlainText -force | ConvertFrom-SecureString 
    
    if($file){
        $secString | Out-File -FilePath $file
    }else{
        return $secString
    }
}
...
Param([string] $file)
    $ss_text = gc $file | ConvertTo-SecureString
    $BSTR = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($ss_text)
