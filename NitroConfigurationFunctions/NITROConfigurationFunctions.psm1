...
#region REST API functions
    ...
    # Added by Sunil Mujumdar to get HA Status and other performance stats available through API
    # Small Tweak to the original function to replace config with stat
    # Statistics documentation available here https://developer-docs.citrix.com/projects/netscaler-nitro-api/en/latest/statistics/load-balancing/lbvserver/lbvserver/
        function Invoke-NSNitroRestApiStat {
        <#
        .SYNOPSIS
            Invoke NetScaler NITRO REST API statistics
        .DESCRIPTION
            Invoke NetScaler NITRO REST API Statistics
        .PARAMETER NSSession
            An existing custom NetScaler Web Request Session object returned by Connect-NSAppliance
        ...
        .EXAMPLE
            Invoke NITRO REST API to add a DNS Server resource.
            $payload = @{ip="10.8.115.210"}
            Invoke-NSNitroRestApiStat -NSSession $Session -OperationMethod POST -ResourceType dnsnameserver -Payload $payload 
        .OUTPUTS
            Only when the OperationMethod is GET:
            PSCustomObject that represents the JSON response content. This object can be manipulated using the ConvertTo-Json Cmdlet.
        .NOTES
        ...
        Write-Verbose "Building URI"
        $uri = "$($Script:NSURLProtocol)://$($NSSession.Endpoint)/nitro/v1/stat/$ResourceType"
        Write-Verbose $uri
        if (-not [string]::IsNullOrEmpty($ResourceName)) {
            $uri += "/$ResourceName"
        }
